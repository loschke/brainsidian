...
### Persona-Block (variantenspezifisch)

{{ONBOARDING_PERSONA_QUESTIONS}}

> Im reinen Boilerplate ist dieser Block leer. Varianten ergänzen hier zwei bis vier zugeschnittene Fragen.
...
- **Pro Projekt / aktuelles Thema (Frage 3):** eine Notiz in `03_PROJECTS/` (`type: projekt`).
- **Pro Mensch (Frage 4):** eine Notiz im passenden Ordner (`type: person`).
- **Pro Ziel (Frage 5):** eine Notiz (`type: ziel`).
- **Ein bis zwei Wissensnotizen** zu Themen, die der Nutzer beiläufig erwähnt hat (`type: notiz` in `02_KNOWLEDGE/`).

Verlinke großzügig: Ein Projekt verweist auf die beteiligten Menschen und das zugehörige Ziel und umgekehrt.
